...
#                                                _storage_c, _functional)
    ...
    def pump_flow_coefficients(self, u=None):
        ...
        if u is None:
            u = np.zeros(self.n_p, dtype=np.float64) # changed this from zeros
            
        # Check max/min head differences
        assert (_dHp_min <= _dHp_max).all()
        print(_alpha_p, _beta_p, _chi_p,  _Qp, u,_z_p,
                                     _dHp_max, _dHp_min, 
                                     _a_p, _b_p, _c_p,
                                     _J_up, 
                                     _J_dp)
    ...
    def prv_flow_coefficients(self, u=None):
        ...
        ##########################################################################
        print(_Qprv, u, _H_set,
                                            _z_prv, _tau_prv, _Cprv_active, _Cprv_open, _Aprv, _y_max_prv, _J_uprv, _J_dprv)
        # Specify orifice heads at previous timestep
        ...
        # a = (cond_0 & cond_1)
        # if a:
        #     _alpha_prv = _gamma_prv_active
        #     _beta_prv = -div_term* _gamma_prv_active
        #     _chi_prv = 0.0
        
        # # Open
        # b = (cond_0 & ~cond_1)
        # if b:
        #     _alpha_prv = _gamma_prv_open
        #     _beta_prv = -_gamma_prv_open
        #     _chi_prv = 0.0
        ...
        # print('from coefficients', 3600*( _alpha_prv*_H_uprv + _beta_prv*_H_dprv), _alpha_prv,_H_uprv , _beta_prv,_H_dprv )
    ...
    def solve_prv_flows(self, dt, u=None):
        ...
        _Qprv_next = numba_solve_prv_flows(H_j, _H_set, u, _z_inv_j, _z_prv, 
                                      _tau_prv, _y_max_prv, _Cprv_open, _Cprv_active, _Aprv, _J_uprv, _J_dprv, g=9.81)
# what's this!!        # TODO: Move this inside numba function
        ...
        # #_Qprv_next = _alpha_prv * _H_uprv + _beta_prv * _H_dprv + _chi_prv
        # print('from solve flows', 3600*_Qprv_next, _alpha_prv,_H_uprv , _beta_prv,_H_dprv)
...
def numba_prv_flow_coefficients(_alpha_prv, _beta_prv, _chi_prv, H_j, _Qprv, u, _z_inv_j, _H_set,
    ...
    # Compute universal coefficients
    _gamma_prv_active = gamma_o(_Qprv, _Aprv, _Cprv_active, g) # use orifice gamma
    # print(_gamma_prv_active)
    # print(_gamma_prv_active[True])
    _gamma_prv_open = gamma_prv(_Qprv, _Aprv, _Cprv_open, g)
...
def numba_solve_prv_flows(H_j, _H_set, u, _z_inv_j, _z_prv, 
    ...
    # _Qprv_next = _alpha_prv * _H_uprv + _beta_prv * _H_dprv + _chi_prv
    # print('from solve flows', _Qprv_next, _alpha_prv,_H_uprv , _beta_prv,_H_dprv)
