...
def apply_EKF(inp, sensors, t_run = 24, dt = 3600, Rcov = None, Rcov_case = 2, Qcov = None, banded = False, num_iter=10, use_tank_init_cond=False, sensor_std_dev=0.1, full_model = None, Qcov_full = None):
    ...
    t1= time.time()
    H_df_model, Q_df_model, Q_pump_model, Q_prv_model, model, Q_in_all_df_model, pumps, superjunctions, orifices, superlinks, prvs = run_pipedream_simulation(inp, t_run, dt, banded = banded)
    t2 = time.time()    
...
#        # open link --> 1, close link --> 0
    ...
    return H_df_filtered, H_df_real, H_df_model, H_df_sensor, model, Q_pump_model, Q_pump_ekf, Q_prv_model, Q_prv_ekf, measurements, Q_df_model, Q_df_filtered
