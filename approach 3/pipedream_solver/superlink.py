...
class SuperLink():
    ...
    def prv_flow_coefficients(self, Hset=None):
        """
        Compute orifice flow coefficients: alpha_uo, beta_uo, chi_uo,
        alpha_do, beta_do, chi_do.
        """
        ...
        # If no input signal, assume orifice is closed
        if Hset is None:
            Hset = np.ones(self.n_prv, dtype=np.float64)
        g = 9.81
        _H_uprv = H_j[_J_uprv]
        _H_dprv = H_j[_J_dprv]
        div_term = _H_uprv / Hset
        # Create indicator functions
    ...
    @safe_divide    
    def solve_prv_flows(self, dt, Hset=None):
        """
        Solve for pump discharges given superjunction heads at time t + dt.
        """
...
# what's this!!        # TODO: Move this inside numba function
    ...
    def _setup_step(self, H_bc=None, Q_in=None, Q_0Ik=None, u_o=None, u_w=None, u_p=None, Hset = None, dt=None,
        ...
        if self.prvs is not None:
            self.prv_flow_coefficients(Hset = Hset)
        self.sparse_matrix_equations(H_bc=H_bc, _Q_0j=Q_in,
                                     first_time=first_time, _dt=dt,
                                     implicit=implicit)

    def _solve_step(self, H_bc=None, Q_in=None, Q_0Ik=None, u_o=None, u_w=None, u_p=None, Hset = None, dt=None,
             first_time=False, implicit=True, banded=False, first_iter=True):
        ...
        if self.prvs is not None:
            self.solve_prv_flows(dt=dt, Hset = Hset)
    ...
    def step(self, H_bc=None, Q_in=None, Q_0Ik=None, u_o=None, u_w=None, u_p=None, Hset=None, dt=None,
        ...
        if banded is None:
            banded = self.banded
        self._setup_step(H_bc=H_bc, Q_in=Q_in, Q_0Ik=Q_0Ik, u_o=u_o, u_w=u_w, u_p=u_p, Hset=Hset, dt=dt,
                         first_time=first_time, implicit=implicit, banded=banded,
                         first_iter=first_iter)
        self._solve_step(H_bc=H_bc, Q_in=Q_in, Q_0Ik=Q_0Ik, u_o=u_o, u_w=u_w, u_p=u_p, Hset=Hset, dt=dt,
                         first_time=first_time, implicit=implicit, banded=banded,
                         first_iter=first_iter)
