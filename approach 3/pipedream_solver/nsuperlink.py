...
class nSuperLink(SuperLink):
    ...
    def prv_flow_coefficients(self, Hset=None):
        ...
        numba_prv_flow_coefficients(_alpha_prv, _beta_prv, _chi_prv, H_j, _Qprv, Hset, 
    ...
    def solve_prv_flows(self, dt, Hset=None):
        ...
        # Compute PRV flows
        # ################################################
        _Qprv_next = numba_solve_prv_flows(H_j, Hset,
...
#     cond_0 = (_H_uprv > _H_dprv)
#     cond_1 = (_H_uprv > _H_set)
#     cond_2 = (np.sqrt(np.abs( _gamma_prv_open * _H_uprv - _gamma_prv_open * _H_dprv)) < epsilon)
#     # Fill coefficient arrays
#     # Active
#     a = (cond_0 & cond_1 & ~cond_2)
#     # _alpha_prv[a] = _gamma_prv_active_us[a]
#     # _beta_prv[a] = - _gamma_prv_active_ds[a]
#     _alpha_prv[a] = _gamma_prv_open[a]
#     _beta_prv[a] = -_gamma_prv_open[a]
#     _chi_prv[a] = 0.0
#     # Open
#     b = (cond_0 & ~cond_1 & ~cond_2)
#     _alpha_prv[b] = _gamma_prv_open[b]
...
@njit(int64(float64[:], float64[:], float64[:], float64[:], float64[:], float64[:], 
            float64[:], float64[:], float64[:], int64[:], int64[:])                                                         ,
      cache=True)
def numba_prv_flow_coefficients(_alpha_prv, _beta_prv, _chi_prv, H_j, _Qprv, Hset,
                                    _Cprv_active, _Cprv_open, _Aprv, _J_uprv, _J_dprv):
    g = 9.81
    _H_uprv = H_j[_J_uprv]
    _H_dprv = H_j[_J_dprv]
...
@njit(float64[:](float64[:], float64[:], 
            float64[:], float64[:], float64[:], int64[:], int64[:], float64),
      cache=True)
def numba_solve_prv_flows(H_j, Hset, 
                             _Cprv_open, _Cprv_active, _Aprv, _J_uprv, _J_dprv, g=9.81):
    # Specify orifice heads at previous timestep
    _H_uprv = H_j[_J_uprv]
    _H_dprv = H_j[_J_dprv]
