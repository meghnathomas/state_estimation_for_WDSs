...
from pipedream_utility import wntr_2_pd
...
def apply_EKF(inp, sensors, t_run = 24, dt = 3600, Rcov = None, Rcov_case = 2, Qcov = None, banded = False, num_iter=10, use_tank_init_cond=False, sensor_std_dev=0.1, full_model = None, Qcov_full = None):
    ...
    for _, valve in wn.valves():
        imp_list.append(valve.start_node_name)
        imp_list.append(valve.end_node_name)
    for _, pump in wn.pumps():
        imp_list.append(pump.start_node_name)
        imp_list.append(pump.end_node_name)
    for reservoir in wn.reservoir_name_list:
        imp_list.append(reservoir)
    for tank in wn.tank_name_list:
        imp_list.append(tank)
    
    # Run pipedream simulation
    ...
    # Extract pipedream parameters
    superjunctions, superlinks, orifices, pumps, prvs, H_bc, Q_in, pats, mult_df, tank_min, tank_max, tank_dict, time_controls_compiled, events_controls_pairs = wntr_2_pd(wn, t_run, dt, imp_list)
    ...
    # Import measurements
    if dt ==3600:
        measurements = pd.read_csv('Networks/ky6_data/ky6_measurements.csv', index_col=0)
    else:
        measurements = pd.read_csv('Networks/ky6_data/ky6_measurements_900.csv', index_col=0)
    measurements  = measurements[sensors]
    ...
    while model.t < (t_run * 3600):    
        ...
        for name in imp_list:
            s_name = name
            H_bc_t[int(list(superjunctions['name']).index(s_name))] = sensor_measurements[name][model.t]+0.05
        count = count+1
    ...
    return H_df_filtered, H_df_model, measurements, model, Q_pump_model, Q_pump_ekf, Q_prv_model, Q_prv_ekf, Q_o_model, Q_o_ekf, Q_df_model, Q_df_filtered