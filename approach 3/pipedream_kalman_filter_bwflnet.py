...
import time
import pipedream_utility_bwfl as pdu
from pipedream_utility_bwfl import wntr_2_pd
import pipedream_simulation_bwfl as pd_sim
from pipedream_simulation_bwfl import *
...
def apply_EKF(inp, sensors, t_run = 24, dt = 3600, Rcov = None, Rcov_case = 2, Qcov = None, banded = False, num_iter=10, use_tank_init_cond=False, sensor_std_dev=0.1, full_model = None, Qcov_full = None):
    ...
    H_df_model, Q_df_model, Q_pump_model, Q_prv_model, Q_o_model, model, Q_in_all_df_model ,pumps, superjunctions, orifices, superlinks, prvs  = run_pipedream_simulation(inp, t_run, dt, banded = banded)
        
    superjunctions, superlinks, orifices, pumps, prvs, H_bc, Q_in, pats, mult_df, tank_min, tank_max, tank_dict, time_controls_compiled, events_controls_pairs = wntr_2_pd(wn, t_run, dt)
    ...
    imp_list = ['node_2012', 'node_0469', 'node_0468', 'node_0837', 'node_0836', 'node_1901', 'node_1900', 'node_0684', 'node_0683', 'node_1420', 'node_1421']
    for element in list(sensor_info['Node/Link']):
        logger_dict[element] = sensor_info['Logger_ID'].loc[sensor_info['Node/Link']==element].values[0]

    sensor_measurements = pd.DataFrame(columns = imp_list, dtype=object) #, index = np.linspace(0,t_run*3600, t_run*4+1))

    for sensor_name in imp_list:
        logger = logger_dict[sensor_name]
        meas_data_all = pd.read_csv(f'Networks/bwflnet-_-data/data/15min/24hr_training_data/{logger}.csv')
        sensor_measurements[sensor_name] = meas_data_all['p_mean'].loc[:t_run*4] + wn.get_node(sensor_name).elevation
    ...
    u_o = np.ones(len(orifices.index))
    ...
    while model.t < (t_run * 3600):    
        ...
        for name in imp_list:
            if name == 'node_2012':
                ...
                s_name = name
            H_bc_t[int(list(superjunctions['name']).index(s_name))] = sensor_measurements[name][count]+0.05
        count = count+1
...
#        # open link --> 1, close link --> 0
    ...
    return H_df_filtered, H_df_model, model, Q_pump_model, Q_pump_ekf, Q_prv_model, Q_prv_ekf, Q_o_model, Q_o_ekf, measurements, Q_df_model, Q_df_filtered
