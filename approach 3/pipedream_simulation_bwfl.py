...
def run_pipedream_simulation(inp, t_run=24, dt=3600, banded=False, num_iter=40, use_tank_init_cond=False):
    ...
    # Store all relevant data points in a dataframe
    ...
    while model.t < (t_run * 3600):    
        ...
        # try to set res head boundary conditions
        # model.bc[int(list(superjunctions['name']).index('node_2548'))] = True
        ...
        count = count+1
        #print(int(list(superjunctions['name']).index('node_2548')))
        #print(model.bc)
...
#        # open link --> 1, close link --> 0
    ...
    return H_df, Q_df, Q_pump, Q_prv, Q_o, model, Q_in_all_df , pumps, superjunctions, orifices, superlinks, prvs
