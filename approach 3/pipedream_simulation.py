...
def run_pipedream_simulation(inp, t_run=24, dt=3600, banded=False, num_iter=40, use_tank_init_cond=False):
    ...
    superjunctions, superlinks, orifices, pumps, prvs, H_bc, Q_in, pats, mult_df, tank_min, tank_max, tank_dict, time_controls_compiled, events_controls_pairs = pdu.wntr_2_pd(wn, t_run, dt, imp_list)
    
    t3 = time.time()
    
    # print(time_controls_compiled)
    ...
    while model.t < (t_run * 3600):    
        ...
        # print(j, model.t, wn.options.time.pattern_timestep, t_run * 3600)
