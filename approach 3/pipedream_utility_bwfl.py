...
def wntr_2_pd(wn,t_run, dt):
    ...
    imp_list = ['node_1925', 'node_1938', 'node_1961', 'node_1971', 'node_1983',
                         ...
                         'node_1876', 'node_2535'] + ['node_0469', 'node_0468', 'node_0837', 'node_0836', 'node_1901', 'node_1900'] +  ['node_0684', 'node_0683', 'node_1420', 'node_1421'] 
    ...
    for i in range(len(junction_names)):
        ...
        if j_type == 'Junction':
            ...
            initial_head[i] = 0
            bc[i] = False
            if junction_names[i] in [ 'node_0469', 'node_0468', 'node_0837', 'node_0836', 'node_1901', 'node_1900', 'node_0684', 'node_0683', 'node_1420', 'node_1421']:
                bc[i] = True
            c[i] = 1e-5
            tank[i] = False
        ...
        if j_type == 'Reservoir':
            ...
            if junction_names[i] == 'node_2548':
                ele[i] = 123.2  #319.1256 # to negate the effects of negative pressures in the model
                bc[i] = True
            initial_head[i] =0.01
            c[i] =  10**(9) #1
            tank[i] = False
    ...
    links.remove('valve_2606')
    links.remove('valve_2605')
    ...
    for i in range(len(links)):
        if links[i] != 'valve_2606' and links[i] != 'valve_2605':
...
#        orifices = orifices.astype(orifices_row.dtypes.to_dict())
    ...
    #Create an orifice connecting the existing tank to the new node    
    orifices_row=pd.DataFrame(columns=['name','id','sj_0','sj_1','A','orientation','z_o','y_max','C'],index=[j], dtype=object)
    orifices_row['name']='valve_2605'
    orifices_row['id']=0
    orifices_row['sj_0']=int(list(superjunctions_m['name']).index(wn.get_link('valve_2605').start_node_name))
    orifices_row['sj_1']=int(list(superjunctions_m['name']).index(wn.get_link('valve_2605').end_node_name))
    orifices_row['A']=1 #0.164
    orifices_row['orientation']='bottom'
    orifices_row['z_o']=0
    orifices_row['y_max']=wn.get_link('valve_2605').diameter
    orifices_row['C']=0.6
    
    orifices=orifices.append(orifices_row)
    
    orifices_row['name']='valve_2606'
    orifices_row['id']=0
    orifices_row['sj_0']=int(list(superjunctions_m['name']).index(wn.get_link('valve_2606').start_node_name))
    orifices_row['sj_1']=int(list(superjunctions_m['name']).index(wn.get_link('valve_2606').end_node_name))
    orifices_row['A']=1
    orifices_row['orientation']='bottom'
    orifices_row['z_o']=0
    orifices_row['y_max']=wn.get_link('valve_2606').diameter
    orifices_row['C']=0.6
    ...
    for valve_name, valve in wn.valves():
        ...
        if valve.valve_type == 'PRV':
            ...
            prv_row['C_active']=0.5
            prv_row['C_open']=0.55
            #prv_row['Hset']= wn.get_node(valve.end_node_name).elevation + valve.setting
            
            # if valve_name == 'link_2214':
            #     prv_row['Hset']= 87.0
            # if valve_name == 'link_2312':
            #     prv_row['Hset']= 86.0
            # if valve_name == 'link_2602':
            #     prv_row['Hset']= 115.0
