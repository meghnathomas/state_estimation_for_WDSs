...
def apply_EKF(inp, sensors, t_run = 24, dt = 3600, Rcov = None, Rcov_case = 2, Qcov = None, banded = False, num_iter=10, use_tank_init_cond=False, sensor_std_dev=0.1, full_model = None, Qcov_full = None):
    ...
    H_df_model, Q_df_model, Q_pump_model, model, Q_in_all_df_model = run_pipedream_simulation(inp, t_run, dt, banded = banded)
    ...
    H_df_real = measurements
    
    #Generate normally distributed random noise within 0.5 m
    #rng = np.random.default_rng(6)
    #vals = rng.normal(0, 0.5, size=measurements.shape)
    #measurements = measurements+vals
    H_df_sensor = measurements
    ...
    if full_model != None:
        ...
        _, _, _, _, H_df_sensor_full, _ = run_pipedream_simulation_sensor(full_model, t_run=t_run, dt=dt, banded=banded, Rcov=Rcov, Qcov=Qcov_full, 
                                                                                                               sensor_std_dev=sensor_std_dev)
        superjunctions_full, _, _, _, _, _, _, _, _, _, _, _, _ = pdu.wntr_2_pd(wn2, t_run, dt)
        sensor_inds_full=[superjunctions_full['name'].to_list().index(s) for s in sensors]
    ...
    if Qcov is None:
        wn = wntr.network.WaterNetworkModel('ky6_sensor.inp')
        sim = wntr.sim.EpanetSimulator(wn)
        results = sim.run_sim()
        Qcov = np.square(np.array(np.std(results.link['flowrate'])))*np.eye(p)    
...
#        # open link --> 1, close link --> 0
    ...
    return H_df_filtered, H_df_real, H_df_model, H_df_sensor, model, Q_superlinks, Q_pump
